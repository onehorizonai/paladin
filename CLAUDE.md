...
| `paladin-pr-review` | `/paladin-pr-review` | Review a diff, PR, staged changes, or uncommitted changes |
...
Every finding must include severity, title, category, evidence, attack scenario, impact, recommended mitigation, required test, and blocking status.
...
Use stable output values: severity is `Medium`, `High`, or `Critical`; check status is `passed`, `failed`, `not applicable`, or `not available`; PR decision is `Approved`, `Approved with non-blocking follow-up`, `Changes requested`, or `Escalated for deeper security review`.
